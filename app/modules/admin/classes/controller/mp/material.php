...
class Controller_MP_Material extends Controller_BaseController
{
     ...
     * @throws \Exception
     */
    public function action_bind_menu(){
        if(\Input::method() == 'POST'){
            \DB::update('wx_accounts_mp_materials')
                ->set([
                    'menu_keyword' => ''
                ])
                ->where('menu_keyword', \Input::post('menu'))
                ->execute();
            $msg = ['status' => 'err', 'msg' => '', 'errcode' => 10];
            $material = \Model_WXAccountMpMaterial::find(\Input::post('id'));
            $material->menu_keyword = \Input::post('menu');
            if($material->save()){
